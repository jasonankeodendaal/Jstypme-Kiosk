    ...
    return (
        <div className="flex flex-col md:flex-row h-[calc(100vh-140px)] bg-slate-50 rounded-3xl border border-slate-200 overflow-hidden shadow-2xl animate-fade-in">
            ...
            <div className="flex-1 overflow-y-auto bg-white relative p-6 md:p-12 lg:p-20 scroll-smooth">
                ...
                {/* Other sections removed for brevity in this XML block */}
...
// PricelistManager and ManualPricelistEditor skipped for brevity
                ...
                <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-20">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                         ...
                         <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                             ...
                             <div className="space-y-4">
                                <div className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100 mb-4">
                                    <div>
                                        <label className="text-xs font-black text-slate-900 uppercase block">Display Fit Mode</label>
                                        <p className="text-[10px] text-slate-400 font-bold uppercase">How images fit the screen</p>
                                    </div>
                                    <div className="flex gap-1 bg-white p-1 rounded-lg border border-slate-200">
                                        <button 
                                            onClick={() => handleLocalUpdate({...localData, screensaverSettings: {...localData.screensaverSettings!, displayStyle: 'contain'}})}
                                            className={`px-3 py-1.5 text-[9px] font-black uppercase rounded ${localData.screensaverSettings?.displayStyle !== 'cover' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                        >Shrink to Fit</button>
                                        <button 
                                            onClick={() => handleLocalUpdate({...localData, screensaverSettings: {...localData.screensaverSettings!, displayStyle: 'cover'}})}
                                            className={`px-3 py-1.5 text-[9px] font-black uppercase rounded ${localData.screensaverSettings?.displayStyle === 'cover' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                        >Fill Screen</button>
                                    </div>
                                </div>
                                {[{ key: 'showProductImages', label: 'Show Products (Images)' }, { key: 'showProductVideos', label: 'Show Products (Videos)' }, { key: 'showPamphlets', label: 'Show Pamphlet Covers' }, { key: 'showCustomAds', label: 'Show Custom Ads' }, { key: 'muteVideos', label: 'Mute Videos' }, { key: 'showInfoOverlay', label: 'Show Title Overlay' }].map(opt => (
                                    <div key={opt.key} className="flex justify-between items-center p-3 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-100">
                                        ...
                                        <button onClick={() => handleLocalUpdate({...localData, screensaverSettings: {...localData.screensaverSettings!, [opt.key]: !(localData.screensaverSettings as any)[opt.key]}})} className={`w-10 h-5 rounded-full transition-colors relative ${(localData.screensaverSettings as any)[opt.key] ? 'bg-blue-600' : 'bg-slate-300'}`}><div className={`w-3 h-3 bg-white rounded-full absolute top-1 transition-all ${(localData.screensaverSettings as any)[opt.key] ? 'left-6' : 'left-1'}`}></div></button>
                                    </div>
                                ))}
                             </div>
                         </div>
                     </div>
                     {/* Rest of Screensaver tab content removed for brevity */}
                </div>
            )}
            {/* Other tabs removed for brevity */}
        </main>
    </div>
  );
};
