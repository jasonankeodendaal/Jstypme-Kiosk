...
const ManualPricelistEditor = ({ pricelist, onSave, onClose }: { pricelist: Pricelist, onSave: (pl: Pricelist) => void, onClose: () => void }) => {
  ...
  const updateItem = (id: string, field: keyof PricelistItem, val: any) => {
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-200 w-16 text-center">Image</th>
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="p-2 text-center">
                    <label className="cursor-pointer group">
                        <div className="w-10 h-10 bg-slate-100 rounded-lg border border-slate-200 flex items-center justify-center overflow-hidden transition-all group-hover:border-blue-500">
                            {uploadingId === item.id ? (
                        ...
                        <input 
                            ...
                            accept="image/*" 
                            onChange={(e) => {
                                if (e.target.files?.[0]) handleItemImageUpload(item.id, e.target.files[0]);
                            }} 
                        />
                    </label>
                  </td>
...
const AdminManager = ({ admins, onUpdate, currentUser }: { admins: AdminUser[], onUpdate: (admins: AdminUser[]) => void, currentUser: AdminUser }) => {
    ...
    const handleAddOrUpdate = () => {
        ...
        if (editingId) {
            ...
            updatedList.push({
                id: generateId('adm'),
                // Fix: Correct variable name from 'name' (window global) to 'newName' (state variable)
                name: newName,
                pin: newPin,
                isSuperAdmin: false,
    ...
    const processAsset = async (zipObj: any, filename: string): Promise<string> => {
        ...
        if (supabase) {
             try {
                 ...
                 // Fix: Type 'void' is not assignable to type 'string'. Explicitly returning the fallback promise to satisfy return type Promise<string>.
                 return new Promise(resolve => {
                    const reader = new FileReader();
                    reader.onloadend = () => resolve(reader.result as string);
...
export const AdminDashboard = ({ storeData, onUpdateData, onRefresh }: { storeData: StoreData | null, onUpdateData: (d: StoreData) => void, onRefresh: () => void }) => {
  ...
  if (!currentUser) return <Auth admins={localData.admins || []} on