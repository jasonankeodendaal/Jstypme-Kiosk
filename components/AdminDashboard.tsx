...
// New Signal Bar component for the Command Center
    ...
    return (
        <div className="flex items-end gap-0.5 h-3">
            {bars.map((threshold, i) => (
                <div 
                    key={i} 
                    className={`w-1 rounded-t-sm transition-all duration-500 ${strength >= threshold ? 'bg-blue-500 shadow-[0_0_5px_rgba(59,130,246,0.5)]' : 'bg-slate-800'}`}
                    style={{ height: `${(i + 1) * 20}%` }}
                />
            ))}
        </div>
...
const ProductEditor = ({ product, onSave, onCancel }: { product: Product, onSave: (p: Product) => void, onCancel: () => void }) => {
    ...
    const addManual = () => {
        const newManual: Manual = { id: generateId('man'), title: "New Manual", images: [], pdfUrl: '', thumbnailUrl: '' };
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <select value={targetBrandId} onChange={(e) => setTargetBrandId(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl font-bold text-sm outline-none focus:border-blue-500 transition-all">
                        ...
                        <div>
                            ...
                            <select value={targetCategoryId} onChange={(e) => setTargetCategoryId(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl font-bold text-sm outline-none focus:border-blue-500 transition-all">
                ...
                <div className="p-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                    <button onClick={onClose} className="px-4 py-2 text-slate-500 font-bold uppercase text-xs">Cancel</button>
                    <button onClick={() => onMove(product, targetBrandId, targetCategoryId)} className="px-6 py-3 bg-blue-600 text-white font-black uppercase text-xs rounded-xl">Confirm Move</button>
                </div>
            </div>
        </div>
    );
            ...
            <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        ...
                        <FileUpload label="Upload Videos" accept="video/*" allowMultiple={true} icon={<Video />} onUpload={(urls: any) => {
                            const newUrls = Array.isArray(urls) ? urls : [urls];
                            setDraft(prev => ({ ...prev, videoUrls: [...prev.videoUrls, ...newUrls] }));
                        }} />
                        <div className="grid grid-cols-1 gap-3 mt-4">
                            {draft.videoUrls.map((url, idx) => (
                                <div key={idx} className="flex items-center gap-4 bg-white p-3 rounded-lg border border-slate-200 group">
                                    <div className="flex-1 min-w-0"><div className="text-xs font-mono truncate text-slate-700">{url.split('/').pop()}</div></div>
                                    <button onClick={() => setDraft({ ...draft, videoUrls: draft.videoUrls.filter((_, i) => i !== idx) })} className="p-1.5 bg-red-50 text-red-500 rounded"><Trash2 size={14} /></button>
                                </div>
                            ))}
                        </div>
                    </div>
    ...
    return (
        <div className="space-y-8">
            <div className="xl:col-span-2 bg-slate-50 p-6 rounded-xl border border-slate-200">
                <h4 className="font-bold text-slate-900 uppercase text-xs mb-6">{editingId ? 'Edit Admin' : 'Add Admin'}</h4>
                ...
                <button onClick={handleAddOrUpdate} className="w-full px-6 py-3 bg-slate-900 text-white rounded-xl text-xs font-bold uppercase hover:bg-slate-800 shadow-lg">{editingId ? 'Update' : 'Add Admin'}</button>
            </div>
            <div className="space-y-4">
                {admins.map(admin => (
                    <div key={admin.id} className="p-4 rounded-xl border bg-white border-slate-200 flex justify-between items-center">
                        <div className="font-bold text-slate-900 text-sm">{admin.name} ({admin.pin})</div>
                        <div className="flex gap-2">
                             ...
                             {admin.id !== 'super-admin' && <button onClick={() => onUpdate(admins.filter(a => a.id !== admin.id))} className="text-red-500"><Trash2 size={14} /></button>}
                        </div>
                    </div>
                ))}
            </div>
...
const importZip = async (file: File, onProgress?: (msg: string) => void): Promise<Brand[]> => {
    ...
    for (const rawPath of Object.keys(loadedZip.files)) {
        ...
        if (fileObj.dir || rawPath.includes('__MACOSX')) continue;
        ...
        const categoryName = parts[1], productName = parts[2], fileName = parts.slice(3).join('/');
    ...
    <div className="flex flex-col h-screen bg-slate-100 overflow-hidden">
        <header className="bg-slate-900 text-white shrink-0 shadow-xl z-20">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800">
                 ...
                 <div className="flex items-center gap-4">
                     <button onClick={handleGlobalSave} disabled={!hasUnsavedChanges} className={`flex items-center gap-2 px-6 py-2 rounded-lg font-black uppercase tracking-widest text-xs transition-all ${hasUnsavedChanges ? 'bg-blue-600 text-white shadow-xl animate-pulse' : 'bg-slate-800 text-slate-500'}`}>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 relative">
            {activeTab === 'fleet' && (
                <div className="animate-fade-in max-w-7xl mx-auto pb-24">
                   <div className="flex items-center justify-between mb-8 bg-slate-900 p-6 rounded-3xl border border-white/5 shadow-2xl">
                       <div className="flex items-center gap-4">
                           ...
                           <div>
                               <h2 className="text-2xl font-black text-white uppercase tracking-tight">Fleet Command Center</h2>
                               <p className="text-blue-400 text-[10px] font-black uppercase tracking-[0.2em]">Real-time Telemetry Dashboard</p>
                           </div>
                       </div>
                       <div className="flex gap-6">
                           <div className="text-right">
                               <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Active Units</div>
                               <div className="text-2xl font-black text-white">{localData.fleet?.length || 0}</div>
                           </div>
                           <div className="text-right border-l border-white/10 pl-6">
                               <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">System Health</div>
                               <div className="text-2xl font-black text-green-500 uppercase">Secure</div>
                           </div>
                       </div>
                   </div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                       {localData.fleet?.map(kiosk => {
                           ...
                           const type = kiosk.deviceType || 'kiosk';
                           
                           return (
                               <div key={kiosk.id} className="bg-slate-950 rounded-[2rem] border border-white/5 shadow-2xl overflow-hidden flex flex-col group relative transition-all hover:scale-[1.02] hover:border-blue-500/30">
                                   <div className={`absolute top-0 left-0 w-full h-1 ${isOnline ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]'}`}></div>
                                   
                                   <div className="p-6 pb-4">
                                       <div className="flex justify-between items-start mb-6">
                                           <div className="flex gap-4">
                                               <div className="p-3 bg-white/5 rounded-2xl border border-white/5 group-hover:text-blue-400 transition-colors">
                                                   {type === 'kiosk' ? <Tablet size={24}/> : type === 'mobile' ? <Smartphone size={24}/> : <Tv size={24}/>}
                                               </div>
                                               <div>
                                                   <h4 className="text-lg font-black text-white uppercase leading-tight truncate max-w-[160px]">{kiosk.name}</h4>
                                                   <div className="flex items-center gap-2 mt-1">
                                                       <span className="text-[10px] font-mono text-slate-500 uppercase font-bold tracking-widest">{kiosk.id}</span>
                                                       <div className="w-1 h-1 rounded-full bg-slate-700"></div>
                                                       <span className="text-[10px] font-mono text-blue-500/80 font-bold uppercase">{kiosk.version}</span>
                                                   </div>
                                               </div>
                                           </div>
                                           <div className="flex flex-col items-end gap-2">
                                               <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border flex items-center gap-2 ${isOnline ? 'bg-green-500/10 text-green-400 border-green-500/20 shadow-[0_0_10px_rgba(34,197,94,0.1)]' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                                                   {isOnline && <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-ping"></div>}
                                                   {isOnline ? 'Active' : 'Offline'}
                                               </div>
                                               <SignalIndicator strength={kiosk.wifiStrength || 0} />
                                           </div>
                                       </div>

                                       <div className="grid grid-cols-2 gap-3 mb-4">
                                           <div className="bg-white/[0.03] border border-white/5 p-3 rounded-2xl">
                                                <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Zone Assignment</div>
                                                <div className="text-xs font-bold text-slate-300 uppercase truncate">{kiosk.assignedZone || 'Terminal-01'}</div>
                                           </div>
                                           <div className="bg-white/[0.03] border border-white/5 p-3 rounded-2xl">
                                                <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Network Trace</div>
                                                <div className="text-xs font-mono font-bold text-slate-400 truncate">{kiosk.ipAddress || '192.168.1.10'}</div>
                                           </div>
                                       </div>
                                   </div>

                                   <div className="mt-auto bg-black/40 p-2 flex gap-2 border-t border-white/5">
                                       <button onClick={() => setEditingKiosk(kiosk)} className="flex-1 py-3 bg-white/5 hover:bg-blue-600/20 text-slate-400 hover:text-blue-400 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 border border-white/5 hover:border-blue-500/30">
                                           <Settings size={14}/> Config
                                       </button>
                                       <button onClick={async () => { if(confirm("Restart Device?")) await supabase.from('kiosks').update({restart_requested: true}).eq('id', kiosk.id); }} className="flex-1 py-3 bg-white/5 hover:bg-orange-600/20 text-slate-400 hover:text-orange-400 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 border border-white/5 hover:border-orange-500/30">
                                           <RefreshCw size={14}/> Reboot
                                       </button>
                                       <button onClick={() => alert("De-Authorizing Device...")} className="px-4 py-3 bg-white/5 hover:bg-red-600/20 text-slate-500 hover:text-red-500 rounded-xl transition-all border border-white/5 hover:border-red-500/30">
                                           <Power size={14}/>
                                       </button>
                                   </div>
                               </div>
            ...
            {activeTab === 'inventory' && (
                !selectedBrand ? (
                       ...
                       {brands.map(brand => (
                           <div key={brand.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg transition-all group relative flex flex-col h-full">
                               ...
                               <div className="p-2 md:p-4">
                                   ...
                                   <button onClick={() => setSelectedBrand(brand)} className="w-full bg-slate-900 text-white py-1.5 md:py-2 rounded-lg font-bold text-[10px] md:text-xs uppercase">Manage</button>
                               </div>
                           </div>
                       ))}
                   </div>
                   ...
                   <div className="animate-fade-in">
                       ...
                       <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2">
                           ...
                           {selectedBrand.categories.map(cat => (
                               <button key={cat.id} onClick={() => setSelectedCategory(cat)} className="bg-white p-2 md:p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md aspect-square flex flex-col justify-center items-center text-center">
                                   <Box size={20} className="mb-2 text-slate-400" />
                                   <h3 className="font-black text-slate-900 uppercase text-[10px] md:text-sm">{cat.name}</h3>
                                </button>
                            ))}
                       </div>
                   </div>
                   ...
                   <div className="animate-fade-in h-full flex flex-col">
                       <div className="flex items-center gap-4 mb-6 shrink-0"><button onClick={() => setSelectedCategory(null)} className="p-2 bg-white border border-slate-200 rounded-lg"><ArrowLeft size={20} /></button><h2 className="text-lg md:text-2xl font-black uppercase text-slate-900 flex-1 truncate">{selectedCategory.name}</h2><button onClick={() => setEditingProduct({ id: generateId('p'), name: '', description: '', specs: {}, features: [], dimensions: [], imageUrl: '' } as any)} className="bg-blue-600 text-white px-3 py-2 md:px-4 rounded-lg font-bold uppercase text-[10px] md:text-xs flex items-center gap-2"><Plus size={14} /> Add</button></div>
                       <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 overflow-y-auto pb-20">
                           {selectedCategory.products.map(product => (
                               <div key={product.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col group">
                                   <div className="aspect-square bg-slate-50 relative flex items-center justify-center p-2 md:p-4">
                                       ...
                                       <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                            <button onClick={() => setEditingProduct(product)} className="p-2 bg-white text-blue-600 rounded-lg font-bold text-xs uppercase">Edit</button>
                                       </div>
                                   </div>
                                   <div className="p-2 md:p-4">
            ...
            {activeTab === 'pricelists' && (
                <PricelistManager 
                    ...
                    onDeletePricelist={(id) => {
                        ...
                        if (toDelete) handleLocalUpdate({ ...localData, pricelists: localData.pricelists?.filter(p => p.id !== id) });
                    }}
                />
            )}
            ...
            {activeTab === 'settings' && (
               <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-20">
                   <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                       ...
                       <FileUpload label="Main Company Logo" currentUrl={localData.companyLogoUrl} onUpload={(url: string) => handleLocalUpdate({...localData, companyLogoUrl: url})} />
                   </div>
                   <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-lg text-white">
                        <button onClick={async () => { if(confirm("WARNING: This will wipe ALL local data and reset to defaults. Continue?")) { const d = await resetStoreData(); setLocalData(d); window.location.reload(); } }} className="w-full p-4 bg-red-900/30 hover:bg-red-900/50 rounded-xl flex items-center justify-center gap-3 transition-colors border border-red-900/50 text-red-300 font-black uppercase tracking-widest text-xs">Factory Reset</button>
                   </div>
               </div>
            )}
        </main>
        ...
        {editingProduct && (
            <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm p-4 md:p-8 flex items-center justify-center animate-fade-in">
                <ProductEditor product={editingProduct} onSave={(p) => { 
                    ...
                    const newProducts = selectedCategory.products.map(x => x.id === p.id ? p : x);
                    if (!selectedCategory.products.find(x => x.id === p.id)) newProducts.push(p);
        ...
        {editingKiosk && (
            <KioskEditorModal kiosk={editingKiosk} onSave={(k) => { setEditingKiosk(null); }} onClose={() => setEditingKiosk(null)} />
        )}
        
        {editingTVModel && (
            <TVModelEditor model={editingTVModel} onSave={(m) => { if (!selectedTVBrand) return; const newModels = selectedTVBrand.models.map(x => x.id === m.id ? m : x); if(!selectedTVBrand.models.find(x => x.id === m.id)) newModels.push(m); const updatedTVBrand = { ...selectedTVBrand, models: newModels }; handleLocalUpdate({ ...localData, tv: { ...localData.tv, brands: tvBrands.map(b => b.id === selectedTVBrand.id ? updatedTVBrand : b) } as TVConfig }); setEditingTVModel(null); }} onClose={() => setEditingTVModel(null)} />
        )}
