...
import { KioskRegistry, StoreData, Brand, Category, Product, AdConfig, AdItem, Catalogue, HeroConfig, ScreensaverSettings, ArchiveData, DimensionSet, Manual, TVBrand, TVConfig, TVModel, AdminUser, AdminPermissions, Pricelist, PricelistBrand, PricelistItem, HeroAsset } from '../types';
...
const downloadZip = async (storeData: StoreData) => {
    ...
    if (mktFolder) {
        // Hero
        if (storeData.hero.assets) {
            for(let i=0; i<storeData.hero.assets.length; i++) {
                await fetchAssetAndAddToZip(mktFolder, storeData.hero.assets[i].url, `hero_asset_${i}`);
            }
        }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const uploadSingle = async (file: File) => {
           ...
           let fileType = file.type.startsWith('video') ? 'video' : file.type === 'application/pdf' ? 'pdf' : file.type.startsWith('audio') ? 'audio' : 'image';
           
           try {
              const url = await uploadFileToStorage(file);
              return { url, base64: localBase64, type: fileType };
           } catch (e) {
              return { url: localBase64, base64: localBase64, type: fileType };
           }
      };
      ...
      try {
          if (allowMultiple) {
              ...
              onUpload(results, types);
          } else {
              const res = await uploadSingle(files[0]);
              setUploadProgress(100);
              onUpload(res.url, res.type, res.base64);
          }
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <select value={targetBrandId} onChange={(e) => setTargetBrandId(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl font-bold text-sm outline-none focus:border-blue-500 transition-all shadow-sm">{allBrands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select>
                        </div>
                        <div>
                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-wider mb-2 ml-1">Destination Category</label>
                            <select value={targetCategoryId} onChange={(e) => setTargetCategoryId(e.target.value)} className="w-full p-3 bg-white border border-slate-300 rounded-xl font-bold text-sm outline-none focus:border-blue-500 transition-all shadow-sm">{targetCategories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                        </div>
                    </div>
                </div>
                <div className="p-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                    <button onClick={onClose} className="px-4 py-2 text-slate-500 font-bold uppercase text-xs">Cancel</button>
                    <button onClick={() => onMove(product, targetBrandId, targetCategoryId)} className="px-6 py-3 bg-blue-600 text-white font-black uppercase text-xs rounded-xl shadow-lg flex items-center gap-2"><Move size={14} /> Confirm Move</button>
                </div>
            </div>
        </div>
    );
            ...
            <div className="bg-white rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <FileUpload label="Cover Image (Optional)" currentUrl={draft.imageUrl} onUpload={(url: any) => setDraft({ ...draft, imageUrl: url })} />
                    </div>
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                        <div className="flex justify-between items-center mb-4">
        ...
        <div className="space-y-8">
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div className="xl:col-span-2 bg-slate-50 p-6 rounded-xl border border-slate-200">
                    <div className="flex justify-between items-center mb-6"><h4 className="font-bold text-slate-900 uppercase text-xs flex items-center gap-2"><UserCog size={16} className="text-blue-600"/> {editingId ? 'Edit Admin Profile' : 'Create New Admin'}</h4>{isEditingSuperAdmin && <span className="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-[10px] font-black uppercase">Super Admin</span>}</div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{PERMISSION_GROUPS.map(group => (<div key={group.title} className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm"><div className="bg-slate-50 p-3 border-b border-slate-100 flex items-center gap-2"><span>{group.icon}</span><span className="text-[10px] font-black uppercase">{group.title}</span></div><div className="p-3 space-y-3">{group.items.map(item => (<label key={item.key} className="flex items-start gap-3 p-2 rounded-lg cursor-pointer hover:bg-slate-50"><input type="checkbox" checked={(newPermissions as any)[item.key]} onChange={(e) => setNewPermissions({...newPermissions, [item.key]: e.target.checked})} className="w-4 h-4 mt-0.5" disabled={isEditingSuperAdmin} /><div><div className="text-xs font-bold text-slate-800">{item.label}</div></div></label>))}</div></div>))}</div>
                    <div className="flex gap-3 pt-4 border-t border-slate-200 mt-4">{editingId && <button onClick={resetForm} className="px-6 py-3 bg-white border border-slate-300 text-slate-600 rounded-xl text-xs font-bold uppercase">Cancel</button>}<button onClick={handleAddOrUpdate} className="flex-1 px-6 py-3 bg-slate-900 text-white rounded-xl text-xs font-bold uppercase flex items-center justify-center gap-2"><Check size={16} /> {editingId ? 'Update' : 'Create'}</button></div>
                </div>
                <div className="space-y-4"><h4 className="font-bold text-slate-900 uppercase text-xs mb-2">Active Admins ({admins.length})</h4><div className="space-y-3">{admins.map(admin => (<div key={admin.id} className="p-4 rounded-xl border bg-white border-slate-200"><div className="flex justify-between items-start"><div><div className="font-bold text-slate-900 text-sm">{admin.name}</div><div className="text-[10px] text-slate-400 font-mono">****</div></div><button onClick={() => startEdit(admin)} className="p-2 text-slate-400 hover:text-blue-600"><Edit2 size={14} /></button></div></div>))}</div></div>
            </div>
        </div>
    );
};
...
export const AdminDashboard = ({ storeData, onUpdateData, onRefresh }: { storeData: StoreData | null, onUpdateData: (d: StoreData) => void, onRefresh: () => void }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans text-slate-900">
      <aside className="w-20 md:w-64 bg-slate-900 text-white flex flex-col shrink-0 z-50">
        <div className="p-4 md:p-6 mb-6"><div className="flex items-center gap-4"><div className="bg-blue-600 p-2 rounded-xl shadow-lg shrink-0"><ShieldCheck size={24} /></div><div className="hidden md:block overflow-hidden"><h2 className="font-black uppercase tracking-tight truncate">Admin Hub</h2><p className="text-[9px] text-blue-400 font-black uppercase tracking-[0.2em]">Kiosk Pro v2.8</p></div></div></div>
        <nav className="flex-1 space-y-1 px-3">{availableTabs.map((tab) => (<button key={tab.id} onClick={() => { setActiveTab(tab.id); setSelectedBrand(null); setSelectedCategory(null); }} className={`w-full text-left p-3 md:px-4 md:py-3 rounded-xl flex items-center gap-4 transition-all duration-200 group ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}><tab.icon size={20} className="shrink-0" /><span className="hidden md:block font-bold text-xs uppercase tracking-wider">{tab.label}</span></button>))}</nav>
        <div className="p-4 border-t border-white/5 space-y-2"><button onClick={() => setCurrentUser(null)} className="w-full p-3 rounded-xl flex items-center gap-4 text-red-400 hover:bg-red-500/10 transition-all group"><Power size={20} className="shrink-0" /><span className="hidden md:block font-bold text-xs uppercase">Logout</span></button></div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-100 relative">
        <header className="h-16 md:h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shrink-0 shadow-sm z-40">
           <div className="flex items-center gap-2 md:gap-4 overflow-hidden"><h2 className="text-sm md:text-xl font-black uppercase text-slate-900 tracking-tight whitespace-nowrap">{availableTabs.find(t => t.id === activeTab)?.label} Control</h2></div>
           <div className="flex items-center gap-2 md:gap-4">{hasUnsavedChanges && <div className="flex items-center gap-2 animate-pulse"><AlertCircle size={16} className="text-orange-500" /></div>}<button onClick={onRefresh} className="p-2 text-slate-400 hover:text-blue-600"><RotateCcw size={20} /></button><button onClick={handleGlobalSave} disabled={!hasUnsavedChanges} className={`px-4 py-2 md:px-6 md:py-3 rounded-xl font-black text-xs uppercase shadow-lg ${hasUnsavedChanges ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-400'}`}>Save To Cloud</button></div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50 relative">
          {activeTab === 'inventory' && (
            <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    {selectedBrand && (<><ChevronRight size={14} className="text-slate-300" /><button onClick={() => setSelectedCategory(null)} className={`px-4 py-2 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all ${selectedBrand && !selectedCategory ? 'bg-slate-900 text-white shadow-md' : 'bg-white text-slate-400 border border-slate-200'}`}>{selectedBrand.name}</button></>)}
                    {selectedCategory && (<><ChevronRight size={14} className="text-slate-300" /><div className="px-4 py-2 rounded-xl bg-slate-900 text-white shadow-md font-black uppercase text-[10px] tracking-widest">{selectedCategory.name}</div></>)}
                </div>
                {!selectedBrand ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        <button onClick={() => { const name = prompt("New Brand Name:"); if(name) handleLocalUpdate({ ...localData!, brands: [...localData!.brands, { id: generateId('b'), name, categories: [] }] }); }} className="bg-white border-2 border-dashed border-slate-300 rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-slate-400 hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all group aspect-square"><Plus size={48}/><span className="font-black uppercase text-sm mt-4">Add New Brand</span></button>
                        {localData?.brands.map(brand => (<div key={brand.id} className="group bg-white rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-2xl transition-all overflow-hidden flex flex-col p-4"><div className="aspect-square bg-slate-50 rounded-[2rem] flex items-center justify-center mb-4 relative overflow-hidden">{brand.logoUrl ? <img src={brand.logoUrl} className="max-w-[70%] max-h-[70%] object-contain" /> : <span className="text-6xl font-black text-slate-200">{brand.name.charAt(0)}</span>}<div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-4"><button onClick={() => setSelectedBrand(brand)} className="bg-white text-slate-900 p-4 rounded-2xl font-black uppercase text-[10px] shadow-xl">Manage Brand</button></div></div><div className="px-4 flex justify-between items-center"><div><h3 className="font-black text-lg text-slate-900 uppercase truncate">{brand.name}</h3></div><button onClick={() => { if(confirm("Delete brand?")) handleLocalUpdate({ ...localData!, brands: localData!.brands.filter(b => b.id !== brand.id) }); }} className="p-2 text-slate-300 hover:text-red-500"><Trash2 size={20} /></button></div></div>))}
                    </div>
                ) : !selectedCategory ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fade-in">
                        <button onClick={() => { const name = prompt("New Category Name:"); if(name) { const updatedBrands = localData!.brands.map(b => b.id === selectedBrand.id ? { ...b, categories: [...b.categories, { id: generateId('c'), name, icon: 'Box', products: [] }] } : b); handleLocalUpdate({ ...localData!, brands: updatedBrands }); } }} className="bg-white border-2 border-dashed border-slate-300 rounded-[2rem] p-6 flex flex-col items-center justify-center text-slate-400 hover:border-purple-500 transition-all aspect-square"><Plus size={32} /><span className="font-black uppercase text-xs mt-2">New Category</span></button>
                        {selectedBrand.categories.map(cat => (<div key={cat.id} className="group bg-white rounded-[2rem] border border-slate-200 shadow-sm p-4 flex flex-col"><div className="aspect-square bg-slate-50 rounded-[1.5rem] flex items-center justify-center mb-4 relative group"><FolderOpen size={48} className="text-slate-200" /><div className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><button onClick={() => setSelectedCategory(cat)} className="bg-white text-slate-900 px-6 py-2 rounded-xl font-black text-[10px] shadow-xl">Manage Models</button></div></div><div className="px-2 flex justify-between items-center"><h3 className="font-bold text-slate-900 uppercase truncate">{cat.name}</h3><button onClick={() => { if(confirm("Delete category?")) { const updatedBrands = localData!.brands.map(b => b.id === selectedBrand.id ? { ...b, categories: b.categories.filter(c => c.id !== cat.id) } : b); handleLocalUpdate({ ...localData!, brands: updatedBrands }); } }} className="p-2 text-slate-300 hover:text-red-500"><Trash2 size={18}/></button></div></div>))}
                        <div className="col-span-full mt-12 bg-white rounded-3xl p-8 border border-slate-200">
                             <h4 className="font-black uppercase text-slate-400 text-xs mb-8 pb-4 border-b border-slate-100">Brand Settings</h4>
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-12"><div className="space-y-6"><InputField label="Official Brand Name" val={selectedBrand.name} onChange={(e:any) => handleLocalUpdate({ ...localData!, brands: localData!.brands.map(b => b.id === selectedBrand.id ? { ...b, name: e.target.value } : b) })} /><div className="flex gap-4"><input type="color" value={selectedBrand.themeColor || '#2563eb'} onChange={(e) => handleLocalUpdate({ ...localData!, brands: localData!.brands.map(b => b.id === selectedBrand.id ? { ...b, themeColor: e.target.value } : b) })} className="w-16 h-12 rounded-lg cursor-pointer bg-white border border-slate-200 p-1" /></div></div><div><FileUpload label="Brand Official Logo" currentUrl={selectedBrand.logoUrl} onUpload={(url: any) => handleLocalUpdate({ ...localData!, brands: localData!.brands.map(b => b.id === selectedBrand.id ? { ...b, logoUrl: url } : b) })} /></div></div>
                        </div>
                    </div>
                ) : (
                    <div className="space-y-6 animate-fade-in">
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                            <button onClick={() => setEditingProduct({ id: generateId('p'), name: '', description: '', specs: {}, features: [], dimensions: [], imageUrl: '' })} className="bg-white border-2 border-dashed border-slate-300 rounded-3xl p-6 flex flex-col items-center justify-center text-slate-400 aspect-square"><Plus size={32} /><span className="font-black uppercase text-[10px] mt-2">Add Product</span></button>
                            {selectedCategory.products.map(product => (<div key={product.id} className="group bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col"><div className="aspect-square bg-white relative p-4 flex items-center justify-center group">{product.imageUrl ? <img src={product.imageUrl} className="max-w-full max-h-full object-contain" /> : <Box size={64}/>}<div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-2 p-4 justify-center"><button onClick={() => setEditingProduct(product)} className="w-full bg-white text-slate-900 py-2 rounded-xl font-black text-[10px] flex items-center justify-center gap-2"><Edit2 size={12}/> Edit</button></div></div><div className="p-4 bg-slate-50/50 flex-1 flex flex-col justify-between"><h4 className="font-bold text-slate-900 uppercase text-xs line-clamp-2">{product.name}</h4><button onClick={() => { const updatedBrands = localData!.brands.map(b => b.id === selectedBrand.id ? { ...b, categories: b.categories.map(c => c.id === selectedCategory.id ? { ...c, products: c.products.filter(p => p.id !== product.id) } : c) } : b); handleLocalUpdate({ ...localData!, brands: updatedBrands }); }} className="text-slate-300 hover:text-red-500 self-end"><Trash2 size={16}/></button></div></div>))}
                        </div>
                    </div>
                )}
            </div>
          ...
          {activeTab === 'marketing' && (
            <div className="max-w-4xl mx-auto space-y-12 animate-fade-in">
                <div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-sm border border-slate-200">
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-8 flex items-center gap-3"><LayoutTemplate className="text-blue-600" /> Hero Branding</h3>
                    <div className="space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200">
                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-wider mb-4">Hero Slideshow Assets (Multi-upload Images/Videos)</label>
                            <FileUpload 
                                label="Add New Hero Background" 
                                accept="image/*,video/*" 
                                allowMultiple={true}
                                onUpload={(urls: any, types: any) => {
                                    const newAssets = Array.isArray(urls) ? urls.map((u, i) => ({ id: generateId('ha'), type: types[i], url: u })) : [{ id: generateId('ha'), type: types, url: urls }];
                                    const currentAssets = localData?.hero.assets || [];
                                    handleLocalUpdate({ ...localData!, hero: { ...localData!.hero, assets: [...currentAssets, ...newAssets] } });
                                }} 
                            />
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                                {(localData?.hero.assets || []).map((asset: HeroAsset) => (
                                    <div key={asset.id} className="relative group aspect-video bg-black rounded-2xl overflow-hidden shadow-md border-2 border-white">
                                        {asset.type === 'video' ? <video src={asset.url} className="w-full h-full object-cover opacity-60" muted /> : <img src={asset.url} className="w-full h-full object-cover" />}
                                        <button 
                                            onClick={() => {
                                                const updatedAssets = (localData?.hero.assets || []).filter((a) => a.id !== asset.id);
                                                handleLocalUpdate({ ...localData!, hero: { ...localData!.hero, assets: updatedAssets } });
                                            }}
                                        ...
                                        ><Trash2 size={14}/></button>
                                        <div className="absolute bottom-2 left-2 px-1.5 py-0.5 rounded bg-black/50 text-white text-[8px] font-black uppercase">{asset.type}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="bg-slate-900 text-white rounded-[3rem] p-8 md:p-12 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <h3 className="text-xl font-black uppercase tracking-tight mb-8">Placements</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {['homeBottomLeft', 'homeBottomRight', 'homeSideVertical', 'homeSideLeftVertical', 'screensaver'].map(zone => (
                                <div key={zone} className="bg-white/5 border border-white/10 rounded-3xl p-6">
                                    <div className="flex justify-between items-center mb-6"><h4 className="font-bold text-xs uppercase tracking-widest text-blue-400">{zone}</h4><span className="text-[10px] uppercase">{(localData?.ads as any)[zone].length}</span></div>
                                    <FileUpload label="Add" accept="image/*,video/*" allowMultiple={true} onUpload={(urls: any, types: any) => { const newAds = Array.isArray(urls) ? urls.map((u, i) => ({ id: generateId('ad'), type: types[i], url: u, dateAdded: new Date().toISOString() })) : [{ id: generateId('ad'), type: types, url: urls, dateAdded: new Date().toISOString() }]; const currentAds = (localData?.ads as any)[zone] || []; handleLocalUpdate({ ...localData!, ads: { ...localData!.ads, [zone]: [...currentAds, ...newAds] } }); }} />
                                    <div className="grid grid-cols-3 gap-3 mt-4">{(localData?.ads as any)[zone].map((ad: AdItem) => (<div key={ad.id} className="relative group aspect-square bg-black rounded-xl overflow-hidden"><video src={ad.url} className="w-full h-full object-cover" muted /><button onClick={() => { const updatedZone = (localData?.ads as any)[zone].filter((a: AdItem) => a.id !== ad.id); handleLocalUpdate({ ...localData!, ads: { ...localData!.ads, [zone]: updatedZone } }); }} className="absolute top-1 right-1 text-white bg-red-500 p-1 rounded-full"><Trash2 size={12}/></button></div>))}</div>
                                </div>
                            ))}
                        </div>
                    </div>
          ...
          {activeTab === 'pricelists' && (<PricelistManager pricelists={localData?.pricelists || []} pricelistBrands={localData?.pricelistBrands || []} onSavePricelists={(newList) => handleLocalUpdate({ ...localData!, pricelists: newList })} onSaveBrands={(newBrands) => handleLocalUpdate({ ...localData!, pricelistBrands: newBrands })} onDeletePricelist={(id) => { const pl = localData?.pricelists?.find(p => p.id === id); if(pl) { handleLocalUpdate({ ...localData!, pricelists: localData!.pricelists!.filter(p => p.id !== id) }); } }} />)}
          {activeTab === 'screensaver' && (<div className="max-w-4xl mx-auto space-y-12 animate-fade-in"><div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-sm border border-slate-200"><h3 className="text-xl font-black uppercase tracking-tight mb-8">Screensaver Timing</h3><InputField label="Idle Timeout" val={localData?.screensaverSettings?.idleTimeout} type="number" onChange={(e:any) => handleLocalUpdate({ ...localData!, screensaverSettings: { ...localData!.screensaverSettings!, idleTimeout: parseInt(e.target.value) } })} /></div></div>)}
          {activeTab === 'guide' && (<SystemDocumentation />)}
        </div>
      </main>

      {editingProduct && selectedBrand && selectedCategory && (
          <div className="fixed inset-0 z-[110] bg-black/60 flex items-center justify-center p-0 md:p-12 animate-fade-in backdrop-blur-md">
              <div className="w-full h-full max-w-6xl overflow-hidden shadow-2xl relative"><ProductEditor product={editingProduct} onSave={(updated) => { const updatedBrands = localData!.brands.map(b => b.id === selectedBrand.id ? { ...b, categories: b.categories.map(c => c.id === selectedCategory.id ? { ...c, products: c.products.find(px => px.id === updated.id) ? c.products.map(px => px.id === updated.id ? updated : px) : [...c.products, updated] } : c) } : b); handleLocalUpdate({ ...localData!, brands: updatedBrands }); setEditingProduct(null); }} onCancel={() => setEditingProduct(null)} /></div>
          </div>
      )}
    </div>
  );
