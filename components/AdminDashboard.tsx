                    ...
                    <div className="space-y-20 animate-fade-in max-w-5xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 flex items-between justify-between">
                                     <div className="flex items-center gap-3">
...
const ManualPricelistEditor = ({ pricelist, onSave, onClose }: { pricelist: Pricelist, onSave: (pl: Pricelist) => void, onClose: () => void }) => {
  ...
  const handleSpreadsheetImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        if (validRows.length === 0) { alert("No data."); return; }
    ...
    <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        <div className="p-6 border-b border-slate-100 bg-slate-50 flex flex-col md:flex-row justify-between gap-4 shrink-0">
          ...
          <div className="flex flex-wrap gap-2">
            <label className="bg-slate-900 text-white px-4 py-2 rounded-xl font-black text-xs uppercase flex items-center gap-2 hover:bg-slate-800 transition-colors shadow-lg cursor-pointer">
              {isImporting ? <Loader2 size={16} className="animate-spin" /> : <FileInput size={16} />} Select File <input type="file" className="hidden" accept=".csv,.xlsx" onChange={handleSpreadsheetImport} />
            </label>
        ...
        <div className="flex-1 overflow-auto p-6">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b">SKU</th>
                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b">Description</th>
                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b">Normal Price</th>
                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b">Promo Price</th>
                <th className="p-3 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b w-10"></th>
              </tr>
            </thead>
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  <td className="p-2"><input value={item.sku} onChange={(e) => updateItem(item.id, 'sku', e.target.value)} className="w-full p-2 bg-transparent border-b outline-none text-sm uppercase" /></td>
                  <td className="p-2"><textarea value={item.description} onChange={(e) => updateItem(item.id, 'description', e.target.value)} className="w-full p-2 bg-transparent border-b outline-none text-sm resize-none h-12" /></td>
                  <td className="p-2"><input value={item.normalPrice} onBlur={(e) => handlePriceBlur(item.id, 'normalPrice', e.target.value)} onChange={(e) => updateItem(item.id, 'normalPrice', e.target.value)} className="w-full p-2 bg-transparent border-b outline-none text-sm" /></td>
                  <td className="p-2"><input value={item.promoPrice} onBlur={(e) => handlePriceBlur(item.id, 'promoPrice', e.target.value)} onChange={(e) => updateItem(item.id, 'promoPrice', e.target.value)} className="w-full p-2 bg-transparent border-b outline-none text-sm text-red-600" /></td>
        ...
        <div className="p-4 border-t bg-slate-50 flex justify-end gap-3">
          ...
          <button onClick={() => { onSave({ ...pricelist, items, type: 'manual', dateAdded: new Date().toISOString() }); onClose(); }} className="px-8 py-3 bg-blue-600 text-white font-black uppercase text-xs rounded-xl shadow-lg hover:bg-blue-700">Save Builder</button>
        </div>
      </div>
    </div>
  );
...
const PricelistManager = ({ pricelists, pricelistBrands, onSavePricelists, onSaveBrands, onDeletePricelist }: { pricelists: Pricelist[], pricelistBrands: PricelistBrand[], onSavePricelists: (p: Pricelist[]) => void, onSaveBrands: (b: PricelistBrand[]) => void, onDeletePricelist: (id: string) => void }) => {
    ...
    const addBrand = () => {
        ...
        onSaveBrands([...pricelistBrands, newBrand]); setSelectedBrand(newBrand);
    };
    const updateBrand = (id: string, updates: Partial<PricelistBrand>) => {
        onSaveBrands(pricelistBrands.map(b => b.id === id ? { ...b, ...updates } : b));
    ...
    return (
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6 h-[calc(100vh-140px)]">
             <div className="w-full md:w-1/3 h-full flex flex-col bg-white border rounded-2xl shadow-sm overflow-hidden shrink-0">
                 <div className="p-4 bg-slate-50 border-b flex justify-between items-center">
                     <h2 className="font-black text-slate-900 uppercase text-xs">Pricelist Brands</h2>
                     <button onClick={addBrand} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg font-bold text-[10px] uppercase"><Plus size={12} /> Add</button>
                 </div>
                 <div className="flex-1 overflow-y-auto p-2 space-y-2">
                     {sortedBrands.map(brand => (
                         <div key={brand.id} onClick={() => setSelectedBrand(brand)} className={`p-3 rounded-xl border cursor-pointer ${selectedBrand?.id === brand.id ? 'bg-blue-50 border-blue-300' : 'bg-white border-slate-100'}`}>
                             <div className="flex items-center gap-3">
                                 <div className="w-10 h-10 bg-white rounded-lg border flex items-center justify-center shrink-0 overflow-hidden">
                             ...
                             {selectedBrand?.id === brand.id && (
                                 <div className="mt-3 pt-3 border-t space-y-2" onClick={e => e.stopPropagation()}>
                                     <input value={brand.name} onChange={(e) => updateBrand(brand.id, { name: e.target.value })} className="w-full text-xs font-bold p-1 border-b outline-none bg-transparent" />
             ...
             <div className="flex-1 h-full bg-slate-50 border rounded-2xl p-4 flex flex-col overflow-hidden">
                 <div className="flex justify-between items-center mb-4 shrink-0">
                     <h3 className="font-bold text-slate-700 uppercase text-xs tracking-wider">{selectedBrand?.name || 'Select Brand'}</h3>
                     <button onClick={addPricelist} disabled={!selectedBrand} className="bg-green-600 text-white px-4 py-2 rounded-lg font-bold text-xs uppercase flex items-center gap-2"><Plus size={12} /> Add List</button>
                 </div>
                 <div className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-4">
                     {filteredLists.map((item) => (
                         <div key={item.id} className="rounded-xl border shadow-sm bg-white p-4 flex flex-col gap-3">
                             <input value={item.title} onChange={(e) => updatePricelist(item.id, { title: e.target.value, dateAdded: new Date().toISOString() })} className="w-full font-bold text-slate-900 border-b outline-none text-sm" />
                             <div className="grid grid-cols-2 gap-2">
                                 <select value={item.month} onChange={(e) => updatePricelist(item.id, { month: e.target.value })} className="text-xs p-1 border rounded">{months.map(m => <option key={m} value={m}>{m}</option>)}</select>
                                 <input type="number" value={item.year} onChange={(e) => updatePricelist(item.id, { year: e.target.value })} className="text-xs p-1 border rounded" />
                             </div>
                             <div className="flex gap-2">
                                 <button onClick={() => updatePricelist(item.id, { type: 'pdf' })} className={`flex-1 py-1 text-[10px] font-black rounded ${item.type !== 'manual' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-400'}`}>PDF</button>
                                 <button onClick={() => updatePricelist(item.id, { type: 'manual' })} className={`flex-1 py-1 text-[10px] font-black rounded ${item.type === 'manual' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}`}>TABLE</button>
                             </div>
                             {item.type === 'manual' ? (
                                <button onClick={() => setEditingManualList(item)} className="py-2 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black uppercase">Open Builder</button>
                             ) : (
                                <FileUpload label="PDF" accept="application/pdf" icon={<FileText />} currentUrl={item.url} onUpload={(url: any) => updatePricelist(item.id, { url: url })} />
                             )}
                             <button onClick={() => onDeletePricelist(item.id)} className="text-red-500 text-[10px] font-bold uppercase"><Trash2 size={12} className="inline mr-1" /> Delete</button>
                         </div>
                     ))}
                 </div>
             </div>
...
/**
 * Main AdminDashboard component coordinating sub-modules for retail management.
 * Provides named export as expected by App.tsx.
 */
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ storeData, onUpdateData, onRefresh }) => {
    ...
    const canAccess = (permission: keyof AdminPermissions) => {
        if (!activeUser) return false;
        if (activeUser.isSuperAdmin) return true;
        return activeUser.permissions[permission];
    };

    if (!activeUser) {
        return <Auth admins={storeData.admins} onLogin={setActiveUser} />;
    }

    const handleSave = async (updatedData: StoreData) => {
        setIsSaving(true);
        try {
    ...
    const tabs = [
        { id: 'inventory', label: 'Inventory', icon: <Box size={18} />, permission: 'inventory' as keyof AdminPermissions },
        { id: 'pricelists', label: 'Pricelists', icon: <RIcon size={18} />, permission: 'pricelists' as keyof AdminPermissions },
        { id: 'marketing', label: 'Marketing', icon: <Megaphone size={18} />, permission: 'marketing' as keyof AdminPermissions },
        { id: 'tv', label: 'TV Mode', icon: <Tv size={18} />, permission: 'tv' as keyof AdminPermissions },
        { id: 'screensaver', label: 'Screensaver', icon: <Monitor size={18} />, permission: 'screensaver' as keyof AdminPermissions },
        { id: 'fleet', label: 'Fleet', icon: <Activity size={18} />, permission: 'fleet' as keyof AdminPermissions },
        { id: 'settings', label: 'Settings', icon: <Settings size={18} />, permission: 'settings' as keyof AdminPermissions },
        { id: 'docs', label: 'Docs', icon: <BookOpen size={18} />, permission: 'history' as keyof AdminPermissions },
    ];
            ...
            <div className="w-64 bg-slate-900 flex flex-col shrink-0">
                <div className="p-6 border-b border-white/5">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="bg-blue-600 p-2 rounded-xl"><ShieldCheck className="text-white" size={24} /></div>
                        <div>
                            <h1 className="text-white font-black uppercase text-sm tracking-tighter">Admin Dashboard</h1>
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Kiosk Pro v2.8</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/5">
                        <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-white font-black text-xs">{activeUser.name.charAt(0)}</div>
                        <div className="min-w-0">
                            <div className="text-white font-bold text-xs truncate">{activeUser.name}</div>
                            <div className="text-[9px] text-slate-500 font-bold uppercase truncate">{activeUser.isSuperAdmin ? 'Super Admin' : 'Manager'}</div>
                        </div>
                    </div>
                </div>
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                    {tabs.filter(t => canAccess(t.permission)).map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
                            {tab.icon} <span className="font-bold text-xs uppercase tracking-widest">{tab.label}</span>
                        </button>
                    ))}
                </nav>
                <div className="p-4 border-t border-white/5">
                    <button onClick={() => setActiveUser(null)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-all"><LogOut size={18} /><span className="font-bold text-xs uppercase tracking-widest">Sign Out</span></button>
                </div>
            </div>
            <div className="flex-1 flex flex-col overflow-hidden">
                <header className="h-20 bg-white border-b border-slate-200 px-8 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-4">
                        <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">{tabs.find(t => t.id === activeTab)?.label}</h2>
                        <button onClick={onRefresh} className="p-2 text-slate-400 hover:text-blue-600"><RefreshCw size={18} /></button>
                    </div>
                    <div className="flex items-center gap-4">
                        <button onClick={() => setShowSetupGuide(true)} className="flex items-center gap-2 text-slate-500 hover:text-slate-900 font-bold uppercase text-[10px] tracking-widest px-4 py-2"><Info size={16} /> Guide</button>
                        <button onClick={() => window.location.href = '/'} className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 transition-all"><Monitor size={16} /> Kiosk</button>
                    </div>
                </header>
                <main className="flex-1 overflow-y-auto p-8">
                    {activeTab === 'inventory' && (
                        <div className="bg-white p-12 rounded-[2rem] border-2 border-dashed border-slate-200 text-center flex flex-col items-center justify-center min-h-[400px]">
                            <Box size={64} className="text-slate-200 mb-4" />
                            <p className="font-black text-slate-300 uppercase tracking-[0.2em]">Product Inventory Module</p>
                            <p className="text-xs text-slate-400 mt-2">Manage brands, categories and catalog associations.</p>
                        </div>
                    )}
                    {activeTab === 'pricelists' && (
                        <PricelistManager 
                            ...
                            onSavePricelists={(p) => handleSave({ ...storeData, pricelists: p })}
                            onSaveBrands={(b) => handleSave({ ...storeData, pricelistBrands: b })}
                            onDeletePricelist={(id) => handleSave({ ...storeData, pricelists: storeData.pricelists?.filter(p => p.id !== id) })}
                        />
                    )}
                    {activeTab === 'docs' && <SystemDocumentation />}
                    {['marketing', 'tv', 'screensaver', 'fleet', 'settings'].includes(activeTab) && (
                         <div className="flex flex-col items-center justify-center h-full text-slate-200">
                             <Settings size={80} className="mb-4 opacity-10" />
                             <p className="font-black uppercase tracking-widest text-lg">{activeTab} system placeholder</p>
                         </div>
                    )}
                </main>
                {isSaving && (
                    <div className="fixed bottom-8 right-8 bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border border-white/10 z-[200]">
                        <Loader2 className="animate-spin text-blue-400" size={20} />
                        <span className="font-black uppercase text-xs tracking-widest">Syncing Cloud</span>
                    </div>
                )}
            </div>
