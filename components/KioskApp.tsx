...
const ManualPricelistViewer = ({ pricelist, onClose, companyLogo, brandLogo, brandName }: { pricelist: Pricelist, onClose: () => void, companyLogo?: string, brandLogo?: string, brandName?: string }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-slate-900/95 backdrop-blur-md flex flex-col items-center justify-center p-0 md:p-8 animate-fade-in print:bg-white print:p-0 print:block overflow-hidden print:overflow-visible" onClick={onClose}>
      ...
      <div className="viewer-container relative w-full max-w-7xl bg-white rounded-[2rem] shadow-2xl overflow-hidden max-h-full flex flex-col transition-all print:rounded-none print:shadow-none print:max-h-none print:block" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between shrink-0 print:hidden">
            <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-300 rounded-lg text-xs font-bold uppercase disabled:opacity-50 hover:bg-slate-50"
            >
      ...
      {enlargedImage && (
          <div className="absolute inset-0 z-50 bg-black/90 flex items-center justify-center p-8 animate-fade-in" onClick={() => setEnlargedImage(null)}>
              <img src={enlargedImage} className="max-w-full max-h-full object-contain rounded-lg shadow-2xl" />
...
export const KioskApp = ({ storeData, lastSyncTime, onSyncRequest }: { storeData: StoreData | null, lastSyncTime: string, onSyncRequest: () => void }) => {
  ...
  const [view, setView] = useState<'brands' | 'category' | 'products' | 'detail'>('brands');
  const [selectedBrand, setSelectedBrand] = useState<Brand | null>(null);
  const [selectedCategory, setSelectedCategory] = useState<Category | null>(null);
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
  
  const [viewingCatalog, setViewingCatalog] = useState<Catalogue | null>(null);
  ...
  useEffect(() => {
      const interval = setInterval(async () => {
          const remoteCommands = await sendHeartbeat();
          if (remoteCommands) {
              if (remoteCommands.deviceType && remoteCommands.deviceType !== deviceType) {
                  setDeviceType(remoteCommands.deviceType as any);
              }
              if (remoteCommands.restart) {
  ...
  const resetIdleTimer = useCallback(() => {
      ...
      if (screensaverEnabled && storeData?.screensaverSettings) {
          idleTimer.current = window.setTimeout(() => {
              setIsScreensaverActive(true);
              // Reset View Stack on Screensaver Activation
              setView('brands');
              ...
              setCompareList([]);
          }, storeData.screensaverSettings.idleTimeout * 1000);
      }
  }, [screensaverEnabled, isScreensaverActive, storeData?.screensaverSettings]);
  ...
  if (!isSetup) {
      return <SetupScreen storeData={storeData || { brands: [], hero: { title: '', subtitle: '' } } as any} onComplete={() => { setIsSetup(true); setDeviceType(getDeviceType()); }} />;
  }

  if (deviceType === 'tv') {
      return (
          <TVMode 
              storeData={storeData || { brands: [], tv: { brands: [] } } as any} 
  ...
  const renderContent = () => {
      if (view === 'brands') {
          return (
              <BrandGrid 
                  ...
                  onViewGlobalCatalog={(c) => setViewingCatalog(c)}
      ...
      if (view === 'categories' && selectedBrand) {
          return (
              <CategoryGrid 
                  brand={selectedBrand} 
                  storeCatalogs={storeData?.catalogues}
                  onSelectCategory={(c) => { setSelectedCategory(c); setView('products'); }}
                  onViewCatalog={(c) => setViewingCatalog(c)}
                  onBack={() => { setView('brands'); setSelectedBrand(null); }}
                  screensaverEnabled={screensaverEnabled}
                  onToggleScreensaver={() => setScreensaverEnabled(!screensaverEnabled)}
              />
          );
      }
      if (view === 'products' && selectedCategory && selectedBrand) {
          return (
              <ProductList 
                  brand={selectedBrand}
                  ...
                  onBack={() => { setView('categories'); setSelectedCategory(null); }}
                  onViewCatalog={() => {}}
                  screensaverEnabled={screensaverEnabled}
                  onToggleScreensaver={() => setScreensaverEnabled(!screensaverEnabled)}
                  selectedForCompare={compareList}
                  onToggleCompare={(p) => {
                      if (compareList.includes(p.id)) setCompareList(prev => prev.filter(id => id !== p.id));
                      else if (compareList.length < 3) setCompareList(prev => [...prev, p.id]);
                      else alert("Max 3 items for comparison.");
                  }}
  ...
  // --- UI RENDER ---
  return (
    <div className="h-screen flex flex-col bg-slate-100 overflow-hidden font-sans text-slate-900 select-none">
        {/* Header Strip - Only show on Brands/Home view or if needed globally */}
        {view === 'brands' && (
            <div className="bg-slate-900 text-white px-4 py-2 flex justify-between items-center text-[10px] font-bold uppercase tracking-widest shrink-0 z-30 shadow-md">
                <div className="flex items-center gap-4">
                    <span className="flex items-center gap-1.5"><Wifi size={12} className={checkCloudConnection ? "text-green-400" : "text-red-400"} /> {getShopName() || 'Kiosk'}</span>
                    <span className="text-slate-500">|</span>
                    <span className="flex items-center gap-1.5 text-slate-400"><Clock size={12} /> Last Sync: {lastSyncTime || 'Just now'}</span>
                </div>
                <div className="flex items-center gap-4">
                    <button onClick={onSyncRequest} className="hover:text-blue-400 transition-colors flex items-center gap-1"><RefreshCw size={12} /> Sync</button>
                    <button onClick={() => setScreensaverEnabled(!screensaverEnabled)} className={`hover:text-white transition-colors flex items-center gap-1 ${screensaverEnabled ? 'text-green-400' : 'text-slate-500'}`}>
                        {screensaverEnabled ? <MonitorPlay size={12} /> : <MonitorStop size={12} />} Saver
                    </button>
                    <button onClick={() => window.location.href = '/admin'} className="hover:text-white text-slate-500 transition-colors flex items-center gap-1 ml-2"><Lock size={12} /> Admin</button>
                </div>
            </div>
        )}
        ...
        {/* Floating Pricelist Button (Visible mostly everywhere except detail?) */}
        {view !== 'detail' && storeData?.pricelists && storeData.pricelists.length > 0 && (
            <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-2 pointer-events-none">
                ...
                <div className="pointer-events-auto flex flex-col gap-2 items-end">
                    {storeData.pricelists.map(pl => {
                        const isNew = isRecent(pl.dateAdded);
                        return (
                            <button 
                                key={pl.id}
                                onClick={() => pl.type === 'manual' ? setViewingPricelist(pl) : setViewingPdf({ url: pl.url, title: pl.title })}
                                className="bg-slate-900 text-white pl-4 pr-2 py-2 rounded-l-xl shadow-xl flex items-center gap-3 transition-transform hover:-translate-x-2 border-r-4 border-[#c0810d] group max-w-[200px]"
                            >
                                <div className="flex flex-col items-end min-w-0">
                                    <span className="font-black text-[10px] uppercase truncate w-full text-right">{pl.title}</span>
                                    <span className="text-[8px] font-bold text-slate-400">{pl.month} {pl.year}</span>
                                </div>
                                <div className="bg-[#c0810d] p-1.5 rounded-lg text-white shadow-sm shrink-0">
                                    {pl.type === 'manual' ? <List size={16} /> : <FileText size={16} />}
                                </div>
        ...
        {/* Modals */}
        {viewingCatalog && (
            viewingCatalog.pages && viewingCatalog.pages.length > 0 ? (
                <Flipbook 
                    pages={viewingCatalog.pages} 
                    onClose={() => setViewingCatalog(null)} 
                    catalogueTitle={viewingCatalog.title}
                    startDate={viewingCatalog.startDate}
                    endDate={viewingCatalog.endDate}
                    promoText={viewingCatalog.promoText}
                />
            ) : viewingCatalog.pdfUrl ? (
                <PdfViewer url={viewingCatalog.pdfUrl} title={viewingCatalog.title} onClose={() => setViewingCatalog(null)} />
            ) : null
        )}
        ...
        {isComparing && (
            <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-sm flex flex-col animate-fade-in">
                <div className="p-6 border-b border-white/10 flex justify-between items-center">
                    <h2 className="text-2xl font-black text-white uppercase tracking-tight">Product Comparison</h2>
                    <button onClick={() => setIsComparing(false)} className="p-2 bg-white/10 rounded-full hover:bg-white/20 text-white"><X size={24}/></button>
                </div>
                <div className="flex-1 overflow-x-auto p-8">
                    <div className="flex gap-8 min-w-max mx-auto justify-center">
                        {/* Labels Column */}
                        <div className="w-48 flex flex-col gap-4 pt-[320px] text-right text-slate-400 text-xs font-bold uppercase tracking-widest">
                            <div className="h-10 flex items-center justify-end border-b border-white/5">Price / SKU</div>
                            {(selectedCategory?.products.find(p => compareList[0] === p.id)?.features || []).map((_, idx) => (
                                <div key={idx} className="h-20 flex items-center justify-end border-b border-white/5">Feature {idx + 1}</div>
                            ))}
                            {/* Dynamic Spec Labels based on first product */}
                            {Object.keys(selectedCategory?.products.find(p => compareList[0] === p.id)?.specs || {}).map(key => (
                                <div key={key} className="h-12 flex items-center justify-end border-b border-white/5">{key}</div>
                            ))}
                        </div>

                        {compareList.map(id => {
                            const prod = selectedCategory?.products.find(p => p.id === id);
                            if (!prod) return null;
                            return (
                                <div key={id} className="w-80 flex flex-col gap-4">
                                    <div className="h-[300px] bg-white rounded-2xl p-6 flex flex-col items-center text-center shadow-2xl">
                                        <div className="flex-1 flex items-center justify-center mb-4">
                                            {prod.imageUrl ? <img src={prod.imageUrl} className="max-h-full max-w-full object-contain" /> : <Package size={48} className="text-slate-300" />}
                                        </div>
                                        <h3 className="font-black text-slate-900 uppercase text-sm leading-tight">{prod.name}</h3>
                                    </div>
                                    
                                    <div className="h-10 flex items-center justify-center border-b border-white/10 font-mono text-white">{prod.sku || '--'}</div>
                                    
                                    {(prod.features || []).map((f, i) => (
                                        <div key={i} className="h-20 flex items-center justify-center border-b border-white/10 text-white text-xs text-center font-medium px-4 bg-white/5 rounded-lg">
                                            {typeof f === 'string' ? f : 'Feature'}
                                        </div>
                                    ))}

                                    {Object.entries(prod.specs || {}).map(([key, val]) => (
                                        <div key={key} className="h-12 flex items-center justify-center border-b border-white/10 text-blue-300 text-xs font-bold uppercase">
                                            {val}
                                        </div>
                                    ))}
                                </div>
