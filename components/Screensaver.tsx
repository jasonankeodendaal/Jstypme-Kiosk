...
const Screensaver: React.FC<ScreensaverProps> = ({ products, ads, pamphlets = [], onWake, settings }) => {
  ...
  const shouldIncludeItem = (dateString: string | undefined, forceShow = false): boolean => {
      if (forceShow || !dateString) return true;
  ...
  useEffect(() => {
    const buildList = (ignoreAging = false) => {
        const list: PlaylistItem[] = [];
        if (config.showCustomAds) {
            ads.forEach((ad, i) => {
                if (shouldIncludeItem(ad.dateAdded, ignoreAging)) {
                    for(let c=0; c<3; c++) {
                        list.push({
                            id: `ad-${ad.id}-${i}-${c}`,
                            type: ad.type,
        ...
        if (config.showPamphlets) {
            pamphlets.forEach((pamphlet) => {
                if (pamphlet.pages && pamphlet.pages.length > 0) {
                    if (ignoreAging || !pamphlet.endDate || shouldIncludeItem(pamphlet.startDate, ignoreAging)) {
                        list.push({
        ...
        products.forEach((p) => {
            if (!ignoreAging && !shouldIncludeItem(p.dateAdded, ignoreAging)) return;
            ...
            if (config.showProductVideos) {
                if (p.videoUrl) {
                     list.push({ id: `prod-vid-${p.id}`, type: 'video', url: p.videoUrl, title: p.brandName, subtitle: `${p.name} - Official Video`, dateAdded: p.dateAdded });
                }
                if (p.videoUrls) {
                    p.videoUrls.forEach((url, idx) => {
                        if (url !== p.videoUrl) {
                            list.push({ id: `prod-vid-${p.id}-${idx}`, type: 'video', url: url, title: p.brandName, subtitle: `${p.name} - Video Showcase`, dateAdded: p.dateAdded });
                        }
                    });
                }
            }
    ...
    let finalList = buildList(false);
    
    // Safety: If aging logic emptied the list, force all items to show
    if (finalList.length === 0 && (products.length > 0 || ads.length > 0 || pamphlets.length > 0)) {
        finalList = buildList(true);
    }
  ...
  useEffect(() => {
    ...
    if (currentItem.type === 'image') setAnimationEffect(imageEffects[Math.floor(Math.random() * imageEffects.length)]);
    else setAnimationEffect(videoEffects[Math.floor(Math.random() * videoEffects.length)]);
  }, [currentItem?.id]);
  ...
  return (
    <div onClick={onWake} className="fixed inset-0 z-[100] bg-black cursor-pointer flex items-center justify-center overflow-hidden">
      ...
      `}</style>
      <div key={`bg-${currentItem.id}`} className="absolute inset-0 z-0 bg-cover bg-center opacity-40 transition-all duration-1000 bg-parallax blur-2xl" style={{ backgroundImage: `url(${currentItem.url})` }} />
             ...
             <div className="absolute bottom-12 left-8 md:bottom-20 md:left-20 max-w-[80%] md:max-w-[70%] pointer-events-none z-30">
                {currentItem.title && (
                    <div className="slide-up">
                        <h1 className="text-2xl sm:text-3xl md:text-5xl font-black text-white uppercase tracking-tighter drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] mb-2 leading-tight opacity-95 break-words">{currentItem.title}</h1>
                ...
                <div className="flex flex-wrap gap-2 sm:gap-4 items-center slide-up-delay">
                    {currentItem.subtitle && (
                        <div className="bg-white/10 backdrop-blur-md border-l-2 sm:border-l-4 border-blue-500 px-3 sm:px-5 py-2 sm:py-3 shadow-xl rounded-r-lg">
                            <h2 className="text-sm sm:text-lg md:text-2xl font-bold text-white tracking-wide uppercase leading-tight">{currentItem.subtitle}</h2>
                        </div>
                    )}
