...
import { X, Database, Terminal, Copy, Check, Info, ShieldCheck, Lock, Network, Zap, Shield, Cpu as CpuIcon } from 'lucide-react';
...
-- 3. PERMISSIONS (CRITICAL FOR SUCCESSFUL SAVES)
-- Without these, Supabase rejects anonymous writes.
            ...
            {activeTab === 'supabase' && (
                <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
                    <div className="bg-slate-900 rounded-[2rem] p-8 border border-white/5 shadow-2xl">
                        <h2 className="text-white text-2xl font-black uppercase mb-4 flex items-center gap-3"><Terminal size={24} className="text-blue-500" /> Cloud Handshake SQL</h2>
                        <p className="text-slate-400 text-sm mb-6 leading-relaxed">Run this script in the Supabase **SQL Editor** to fix persistence issues. This ensures the database accepts JSON updates for titles and products.</p>
                    ...
                    <div className="bg-blue-600/10 border-2 border-blue-600/20 p-8 rounded-[2rem] text-blue-100">
                        <div className="flex items-center gap-3 mb-4 font-black uppercase tracking-widest"><Zap size={20}/> Why this fixes it:</div>
                        <p className="text-sm font-medium leading-relaxed">The system currently "falls back" because the Cloud rejects anonymous writes without explicit <code>GRANT</code> permissions. Running this SQL allows your Admin panel to securely push JSON objects to the backbone.</p>
                    </div>
                </div>
            )}
        </div>
