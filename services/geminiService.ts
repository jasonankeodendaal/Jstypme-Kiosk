...
const migrateData = (data: any): StoreData => {
    ...
    // Migrate Screensaver Settings
    if (!data.screensaverSettings) {
        data.screensaverSettings = { ...DEFAULT_DATA.screensaverSettings };
    } else {
        if (typeof data.screensaverSettings.transitionStyle === 'undefined') data.screensaverSettings.transitionStyle = 'cinematic';
        if (typeof data.screensaverSettings.showClock === 'undefined') data.screensaverSettings.showClock = false;
        if (typeof data.screensaverSettings.clockFormat === 'undefined') data.screensaverSettings.clockFormat = '24h';
        if (typeof data.screensaverSettings.enableAmbienceBlur === 'undefined') data.screensaverSettings.enableAmbienceBlur = true;
        if (typeof data.screensaverSettings.marketingPriority === 'undefined') data.screensaverSettings.marketingPriority = 3;
    }
          ...
          if (configResponse.data) {
              ...
              if (fleetResponse.data) {
                  const mappedFleet: KioskRegistry[] = fleetResponse.data.map((k: any) => ({
                      ...
                      assigned_zone: k.assigned_zone,
                      notes: k.notes,
                      restart_requested: k.restart_requested
                  }));
