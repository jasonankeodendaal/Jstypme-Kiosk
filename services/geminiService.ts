    ...
    data.brands.forEach((b: any) => {
        ...
        b.categories.forEach((c: any) => {
            ...
            c.products.forEach((p: any) => {
                ...
                if (p.dimensions && !Array.isArray(p.dimensions)) p.dimensions = [p.dimensions];
            });
        });
    });
...
export const generateStoreData = async (): Promise<StoreData> => {
  ...
  if (supabase) {
      try {
          ...
          console.warn("Cloud connection issue during fetch.");
      }
  }
...
/**
 * Saves store configuration to Cloud.
 * Returns true if the Cloud save was successful.
 */
export const saveStoreData = async (data: StoreData): Promise<boolean> => {
    if (!supabase) initSupabase();
    
    // Clean data for Supabase (exclude fleet as it's a separate table)
    const { fleet, ...dataToSave } = data;

    // 1. Primary Save: Supabase Cloud
    if (supabase) {
        try {
            ...
            if (error) {
                console.error("Supabase rejected the update:", error);
                return false;
            }
        } catch (e) {
            console.error("Network failure during Cloud save:", e);
            return false;
        }
    } else {
        // If no cloud detected, we cannot confirm sync
        return false;
    }

    // 2. Secondary Cache: Local Storage
    try {
        localStorage.setItem(STORAGE_KEY_DATA, JSON.stringify(data));
    } catch (e) {
        console.warn("Local storage cache failed (storage full), but Cloud save succeeded.");
    }
...
export const resetStoreData = async (): Promise<StoreData> => {
    const success = await saveStoreData(DEFAULT_DATA);
    if (!success) throw new Error("Could not reset Cloud data. Check connectivity.");
    return DEFAULT_DATA;
};
